  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 items-start">
      ...
      <div className="flex-1 w-full flex flex-col gap-6">
        ...
        {/* Upload Area */}
        <div 
            className="bg-white dark:bg-gray-800 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-8 flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors shadow-sm"
        ...
        >
            ...
            <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-2">批量上传图片</h3>
            <p className="text-gray-500 dark:text-gray-400 text-sm">支持 JPG, PNG, WebP (单张最大 20MB)</p>
        </div>
        ...
        {files.length > 0 && (
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 flex justify-between items-center">
                    <h3 className="font-bold text-gray-700 dark:text-gray-200">压缩列表 ({files.length})</h3>
                ...
                <div className="divide-y divide-gray-100 dark:divide-gray-700">
                    {files.map(file => (
                        <div key={file.id} className="p-4 flex items-center gap-4 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                            {/* Thumbnail */}
                            <div className="w-16 h-16 bg-gray-100 dark:bg-gray-900 rounded-lg overflow-hidden flex-shrink-0 border border-gray-200 dark:border-gray-700 cursor-pointer" onClick={() => file.status === 'done' && setCompareItem(file)}>
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start mb-1">
                                    <p className="font-medium text-gray-800 dark:text-white truncate pr-2">{file.originalFile.name}</p>
                                    {file.status === 'done' && (
                                ...
                                <div className="text-xs text-gray-500 flex items-center gap-2">
                                    ...
                                    {file.status === 'done' && (
                                        <>
                                            <span>→</span>
                                            <span className="font-bold text-gray-700 dark:text-gray-300">{formatSize(file.compressedFile.size)}</span>
                                        </>
                                    )}
                                </div>
                                {/* Progress */}
                                {file.status === 'compressing' && (
                                    <div className="w-full bg-gray-200 rounded-full h-1.5 mt-2 overflow-hidden">
                                        <div className="bg-purple-600 h-full rounded-full transition-all duration-300" style={{width: `${file.progress}%`}}></div>
                                    </div>
                                )}
                                ...
                                {file.status === 'done' && (
                                    <>
                                        <button 
                                            ...
                                            className="p-2 text-gray-500 hover:text-purple-600 hover:bg-purple-50 rounded-lg transition-colors hidden sm:block"
                                            title="对比预览"
                                        >
      ...
      <div className="w-full lg:w-80 flex-shrink-0 flex flex-col gap-6">
            ...
            <div className="space-y-6">
                ...
                <div>
                    ...
                    <input 
                        ...
                        className="w-full rounded-lg border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm py-2 px-3"
                    />
                </div>
        ...
        {files.length > 0 && (
            <button 
                ...
                className="w-full py-3 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl shadow-lg transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
      ...
      {compareItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setCompareItem(null)}>
            <div className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden w-full max-w-5xl h-[80vh] flex flex-col shadow-2xl" onClick={e => e.stopPropagation()}>
                <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                    <h3 className="font-bold text-lg dark:text-white">画质对比: {compareItem.originalFile.name}</h3>
                    <button onClick={() => setCompareItem(null)} className="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                </div>
                <div className="flex-1 overflow-hidden relative flex flex-col md:flex-row">
                    <div className="flex-1 relative bg-checkered p-4 flex items-center justify-center border-r border-gray-200 dark:border-gray-700">
